...
## Agent活动状态修正规则（仅当"Agent最近活动"非空时适用）
                   重要提示：Agent活动信息是判断任务实际复杂度的关键依据，而非仅仅是上下文参考。用户可能用简短的语句描述一个实际上很复杂的任务，Agent活动信息能揭示任务真实的工作量。
                   1. 如果Agent正在修改代码（edit/write），且涉及多模块、多接口、复杂业务逻辑 -> 以Agent实际工作量为准评分，不应因为用户措辞简短而压低分数
                   2. 如果Agent正在修改代码（edit/write），但仅涉及简单操作（注释、格式、模板） -> 不加分，按用户提问正常评估
                   3. 如果Agent正在执行命令（bash）-> 根据命令的性质判断：编译/测试/部署类命令通常反映任务已进入验证阶段，说明任务有一定复杂度
                   4. 如果Agent派发了子Agent（task）-> 说明任务复杂到需要并行拆解，可适当+5-10分
                   5. 如果Agent仅读取代码未修改 -> 分析读取的文件数量和范围：如果读取了大量文件或跨多个模块搜索，说明任务需要理解系统全局，复杂度不低
                   6. Agent操作中出现的文件读取失败等"错误"属于正常探索行为，不加分
                   7. 综合判断：当用户提问简短但Agent活动显示其正在执行复杂操作时，应基于Agent活动信息上调评分，而非跟随用户措辞给出低分
    ...
    public String buildUserPrompt(List<?> candidates, String userQuestion, String agentActivitySummary) {
        ...
        sb.append("请结合用户提问和Agent活动状态评估复杂度(1-100)和意图。注意：当用户提问简短时，Agent活动信息是判断任务实际复杂度的重要依据，应以Agent实际工作量为准。");
        return sb.toString();
    }
