...
import com.miniapi.router.standalone.mapper.ModelConfigMapper;
 ...
 */
@Component
public class SqliteModelConfigRepository implements ModelConfigRepository {

    private final ModelConfigMapper mapper;

    public SqliteModelConfigRepository(ModelConfigMapper mapper) {
        this.mapper = mapper;
    }

    @Override
    public List<ModelConfig> findByTenantId(Long tenantId) {
        List<ModelConfigDO> list = mapper.selectList(
    ...
    public ModelConfig findByDisplayName(Long tenantId, String displayName) {
        ...
        ModelConfigDO dO = mapper.selectOne(
                new LambdaQueryWrapper<ModelConfigDO>()
                        .eq(ModelConfigDO::getTenantId, tenantId)
